...
public class MedDataInterpreter extends ClusterInterpreter{
	...
	public String interpretCluster(String[] cluster_quantised, String[] headers_clust_stat_elements) {
		...
		// thermohaline conditions: top!=bottom temperature
		if (cluster_quantised.length>10 && cluster_quantised[10].equals(cluster_quantised[8])
				&& cluster_quantised[9].equals(cluster_quantised[7]))
			riskLevelH++;
	...
	public String getrisk(String featurename, String quantisation) {
		...
		if (featurename.contains("land_distance") || featurename.contains("depth")) {
			if (quantisation.equals("Low"))
				risk = "High";
			else if (quantisation.equals("High"))
				risk = "Low";
		}
		return risk;
	}
